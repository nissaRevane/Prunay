...
RSpec.describe Taxation::Lmnp do
  ...
  # 13 200 € de recettes, moins 2 000 € de charges, 1 200 € de charges du régime, 6 400 € d'amortissement.
  describe "#taxable_income" do
    it "deducts everything it really pays and the depreciation it does not" do
      ...
      expect(taxation.taxable_income).to eq(3_600)
    end
  ...
  # 3 600 € imposables : 30 % de barème, et 18,6 % de prélèvements sociaux, car un loyer meublé est un BIC.
  describe "the two levies" do
    it "applies the bracket of the household and the social charges of the furnished letting" do
      expect(taxation.income_tax).to eq(1_080)
      expect(taxation.social_charges_rate).to eq(BigDecimal("18.6"))
      expect(taxation.social_charges).to eq(BigDecimal("669.60"))
      expect(taxation.total).to eq(BigDecimal("1749.60"))
    end
  end

  # Le forfait du micro-BIC laisse 6 600 € imposables là où le réel amorti n'en laisse que 3 600.
  ...
  def attributes
    { rent_excluding_charges: 12_000, provision_for_charges: 1_200, marginal_tax_rate: 30, charges: 2_000,
      monthly_rent: 1_000, purchase_price: 200_000, accounting_fees: 500, furniture_maintenance: 400,
      year: 1 }
  end
end
